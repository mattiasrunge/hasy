    ...
    this.setup = function() {
        ...
        serialPort.on("open", function() {
            ...
            serialPort.on("data", function(data) {
                ...
                var data = getPropertyByCommandLetter(data[0]);
                ...
                if (info.substr(0, 2) === "NG") {
                    eventHandler("error", "Failed to set value for property " + data.name, actionId);
                    return;
                }

                var raw = info.substr(2);
                rawToValue(data.property, raw, function(error, value) {
                    if (error) {
                        for (var n = 0; n < data.property.actions.length; n++) {
                            eventHandler("error", "Failed to set new value.", data.property.actions[n]);
                        }
                    } else {
                        var oldValue = property.value;
                        property.value = value;

                        if (data.property.actions.length === 0) {
                            eventHandler("propertyChange", { oldValue: oldValue, newValue: value });
                        }

                        for (var n = 0; n < data.property.actions.length; n++) {
                            eventHandler("propertyChange", { oldValue: oldValue, newValue: value }, data.property.actions[n]);
                        }

                        console.log("Value updated for property " + data.property.name + " to " + value + " was " + oldValue);
                    }
