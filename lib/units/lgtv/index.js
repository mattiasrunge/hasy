...
module.exports = function(config) {
    ...
    this.read = co(function*(data) {
        let result = yield this.send(data);
    console.log(result);
    return result;
...
//         return match ? match[1] : null;
    ...
    this.send = function(data) {
        return new Promise(function(resolve, reject) {
            serialPort.write(data + "\r", function(error, results) {
                if (error) {
                    return reject(error);
                }
...
/*
                serialPort.on("data", function(data) {
                    ...
                    if (info.substr(0, 2) === "NG") {
                        eventHandler("error", "Failed to set value for property " + name, property.actions);
                        property.actions = [];
                        return;
                    }
                    ...
                    rawToValue(property, raw, function(error, value) {
                        if (error) {
                            eventHandler("error", "Failed to set new value.", property.actions);
                            property.actions = [];
                            return;
                        }
                        ...
                        eventHandler("propertyChange", { oldValue: oldValue, newValue: value, name: name }, property.actions);
                        property.actions = [];

                        console.log("Value updated for property " + name + " to " + value + " was " + oldValue);
                    }.bind(this));
                }.bind(this));*/
            }.bind(this));
        }.bind(this));
    ...
    this.setProperty = function(name, value, actionId) {
        ...
        valueToRaw(property, value, function(error, raw) {
            ...
            serialPort.write(command, function(error, results) {
                if (error) {
                    eventHandler("error", "Failed to send command to serial port", actionId ? [ actionId ] : []);
                    return;
                }
            });
        });
